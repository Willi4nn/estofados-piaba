          ...
          >
            {categories.map((cat) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-[13px] font-medium tracking-wide transition-all duration-300 border ${
                  activeFilter === cat
...
const ProjectCard = memo(
  ({
    ...
    <div
      ...
      className="group relative overflow-hidden aspect-[4/3] rounded-2xl cursor-pointer shadow-sm hover:shadow-2xl hover:shadow-secondary-900/10 transition-all duration-500 border border-border-light bg-surface"
    >
      <Image
      ...
      {project.allImages.length > 1 && (
        <div className="absolute top-4 left-4 z-10 bg-secondary-950/80 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5 shadow-sm border border-white/10">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-6 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-[0.16,1,0.3,1] w-full flex flex-col justify-end h-full">
        <p className="text-[11px] text-primary-500 font-bold uppercase tracking-[0.2em] mb-2 transform -translate-y-2 group-hover:translate-y-0 transition-transform duration-500 delay-100">
          {project.category}
        </p>
        <h4 className="text-white font-serif text-xl md:text-2xl leading-tight transform -translate-y-2 group-hover:translate-y-0 transition-transform duration-500 delay-75">
          {project.title}
        </h4>
      </div>
    </div>
